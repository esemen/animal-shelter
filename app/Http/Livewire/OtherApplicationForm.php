...
class OtherApplicationForm extends Component
{
    use WithFileUploads;
    ...
    public function mount()
    {
        ...
        $other = ($this->application->property['type'] ?? null) && !collect(config('mtar.other_application_form.options.property.type'))->contains($this->application->property['type']);
        $this->propertySelect = $other ? "Other" : $this->application->property['type'] ?? null;

        // Load found site through areas (Other or selected value)
        $other = ($this->application->found_through ?? null) && !collect(config('mtar.other_application_form.options.found_through'))->contains($this->application->found_through);
        $this->foundSelect = $other ? "Other" : $this->application->found_through ?? null;
    }

    public function render()
    {
        return view('livewire.other-application-form')
            ->extends('layouts.app')
            ->section('content');
    }
