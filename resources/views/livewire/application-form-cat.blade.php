            ...
            <div class="cell-sm-10 rd-mailform pt-5">
                ...
                <x-field-section-header nolabel field="state.confirmation.travel">
                    <div class="form-check checkbox ml-4">
                        <input wire:model.defer="state.confirmation.travel" class="form-check-input" type="checkbox"
                               value="1">
                        <p class="form-label-outside">Please confirm you understand that all members of your household are required to travel to collect the cat you are applying for wherever it is located. (See <a class="d-inline" href="{{ url('/adopt-a-pet/dog#adoption-procedures') }}" target="_blank"> Adoption Procedures.</a> for further information).
                        </p>
                    </div>
                </x-field-section-header>
                ...
                <x-field-section-header label="3. What type of property do you live in?"
                    ...
                    <select wire:model.lazy="propertySelect" class="form-control">
                        ...
                        @foreach(config('mtar.cat_application_form.options.property.type') as $option)
                ...
                <x-field-section-header label="4. Do you own or rent your home?"
                    ...
                    <select wire:model.lazy="state.property.ownership" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.property.ownership') as $key => $option)
                ...
                <x-field-section-header label="5. How would you describe the road you live in?" field="state.environment.road">
                    <select wire:model="state.environment.road" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.environment.road') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                </x-field-section-header>
                <x-field-section-header label="6. Will the cat be an indoor or outdoor cat?" field="state.environment.indoor_outdoor">
                    <select wire:model="state.environment.indoor_outdoor" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.environment.indoor_outdoor') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                </x-field-section-header>
                <x-field-section-header label="7. Do you have a cat flap?" field="state.environment.cat_flap">
                    <select wire:model="state.environment.cat_flap" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.environment.cat_flap') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                </x-field-section-header>

                <x-field-section-header label="8. Do you have a back garden? ?"
                                        field="state.garden">
                    <select wire:model="state.garden" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.garden') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                </x-field-section-header>
                ...
                <x-field-section-header field="state.occupants.children.*" nomessage
                                        label="9. Please give the ages of any children (aged under 18) living in the property">
                ...
                <x-field-section-header :field="['state.occupants.adults','state.occupants.adults.*']" nomessage
                                        label="10. Please give the ages of all the adults (aged 18 and over) who live at the property">
                ...
                <x-field-section-header label="11. Please give the occupation and the working hours that
                ...
                <x-field-section-header label="12. Do you work from home?"
                                        :field="['state.occupation.wfh','employerVerification']">
                    <select wire:model="state.occupation.wfh" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.occupation.wfh') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                ...
                <x-field-section-header field="state.experience.other_animals"
                                        label="13. What other animals currently live in your house. Please include breed, sex and age of animals?">
                    <textarea wire:model="state.experience.other_animals" class="form-control"></textarea>
                </x-field-section-header>

                <x-field-section-header :field="['state.experience.reactive_dogs']"
                                        label="14. If you own dog(s) have they met/lived with cats before?">
                    <select wire:model="state.experience.reactive_dogs" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.experience.reactive_dogs') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                </x-field-section-header>
                ...
                <x-field-section-header :field="['state.care.neutered','state.care.not_neutered']"
                                        label="15. If you already own dogs/cats are they all spayed/neutered?">
                    <select wire:model="state.care.neutered" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.care.neutered') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                ...
                <x-field-section-header :field="['state.care.inoculated','state.care.not_inoculated']"
                                        label="16. Are all animals you own inoculated annually and will you continue to inoculate the cat
                    you want to adopt?">
                    <select wire:model="state.care.inoculated" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.care.inoculated') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                ...
                <x-field-section-header field="state.care.not_able_to_care"
                                        label="17. What would happen to the dog if you were no longer able to care for it?">
                    <textarea wire:model="state.care.not_able_to_care" class="form-control"></textarea>
                </x-field-section-header>

                <x-field-section-header :field="['state.care.kitten_neuter','state.care.kitten_not_neuter']"
                                        label="18. If adopting a kitten I agree to have it spayed/neutered by the time it is six months old.">
                    <select wire:model="state.care.kitten_neuter" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.care.kitten_neuter') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                    <div class="pb-3 {{ ($state['care']['kitten_neuter'] ?? '') === 'no' ? null : 'hidden' }}">
                        <p class="px-2">If you've answered no to the above question, please give the reason why.</p>
                        <textarea wire:model="state.care.kitten_not_neuter" class="form-control"></textarea>
                    </div>
                </x-field-section-header>

                <x-field-section-header field="state.care.vet_contact"
                                        label="19. Please give the name, address and telephone number of your vet or the vet you
                ...
                <x-field-section-header label="20. Will you have insurance for your dog or pay as you go?"
                                        field="state.care.insurance">
                    <select wire:model="state.care.insurance" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.care.insurance') as $key => $option)
                            <option value="{{ $key }}">{{ $option }}</option>
                        @endforeach
                    </select>
                </x-field-section-header>
                ...
                <x-field-section-header
                    label="21. Please add any other information you think may be helpful in your application here">
                    <textarea wire:model="state.additional_info" class="form-control"></textarea>
                </x-field-section-header>

                <x-field-section-header label="22. Please confirm any holidays plans you have"
                                        field="state.plans.holiday">
                    @foreach (config('mtar.cat_application_form.options.plans.holiday') as $key => $option)
                ...
                <x-field-section-header label="23. Do you have plans to move home within the next 3 months?"
                                        field="state.plans.move">
                    @foreach (config('mtar.cat_application_form.options.plans.move') as $key => $option)
                        <div class="form-check checkbox">
                ...
                <x-field-section-header field="state.plans.collect"
                                        label="24. Please confirm you will be able to collect the cat you've applied for within 3 days of passing your home check.">
                    @foreach (config('mtar.cat_application_form.options.plans.collect') as $key => $option)
                        <div class="form-check checkbox">
                ...
                <x-field-section-header :field="['foundSelect','state.found_through']"
                                        label="25. Please tell us how you found out about Many Tears. For
                    ...
                    <select wire:model="foundSelect" class="form-control">
                        <option value="">Select</option>
                        @foreach(config('mtar.cat_application_form.options.found_through') as $option)
                            <option value="{{ $option }}">{{ $option }}</option>
                        @endforeach
                    </select>
                ...
                <x-field-section-header nolabel nomessage field="state.confirmations.*">
                    @foreach(config('mtar.cat_application_form.options.confirmations') as $key => $value)
