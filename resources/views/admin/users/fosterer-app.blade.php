...
@section('content')
    <div class="container">
        ...
        <x-users.fosterer-information-card class="mt-4" :application="$application"/>
        <div class="text-center mt-4">
            <span></span>
            <a class="btn btn-primary mr-5" href="{{ route('users.pending') }}"><i class="fa fa-undo"></i> Return</a>
            <form method="post" action="{{ route('users.fosterer.approve', ['application' => $application]) }}" class="d-inline">
                @csrf
                @method('put')
                <button class="btn btn-success"><i class="fa fa-check"></i> Approve</button>
            </form>
            <form method="post" action="{{ route('users.fosterer.reject', ['application' => $application]) }}" class="d-inline">
                @csrf
                @method('delete')
